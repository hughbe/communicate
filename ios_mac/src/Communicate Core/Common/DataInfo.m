...
- (instancetype)initWithData:(NSData *)data {
    ...
    if(self) {
        ...
        if(self.dataType != CommunicationDataTypeTermination) {
            self.headerLength = [DataInfo headerLengthFromHeaderData:data];
...
+ (NSArray *)byteHeaders {
    ...
    dispatch_once(&onceToken, ^{
        byteHeaders = @[[ByteHeader stringByteHeader], [ByteHeader imageByteHeader], [ByteHeader fileByteHeader], [ByteHeader JSONByteHeader], [ByteHeader otherByteHeader], [ByteHeader terminationByteHeader]];
    });
    return byteHeaders;
}
